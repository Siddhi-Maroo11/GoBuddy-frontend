...
export interface MapClickEvent { lat: number; lng: number; }
...
export class DriverMapService {
  ...
  public invalidateSize(): void { this.map?.invalidateSize(); }
  ...
  public async fetchAndDrawRoute(
    ...
    if (!data.routes?.length)
      return { coords: [], distanceKm: 0, durationMin: 0, eta: '' };
    ...
    if (pickupLabel && pickupLat !== undefined && pickupLng !== undefined) {
      ...
      this.placeEtaLabel(pickupLat, pickupLng, eta, distanceKm);
    }
